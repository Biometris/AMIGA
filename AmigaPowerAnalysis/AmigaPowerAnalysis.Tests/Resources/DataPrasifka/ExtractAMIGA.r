...
extract_endpoints_AMIGA <- function (endpoints, anticipated_number_of_periods) {
  le <- length(endpoints)
  ...
  for (i in 1:length(endpoints)) {
    ...
    fit <- tryCatch(glm(rounded_correction_anticipated_number_of_periods ~ 1 + Site*Year, data=endpoint_plot_totals, family=quasipoisson()),
          error=function(e) NULL)
    sink(file.path("Outputs", paste(endpoints[i], "_glm_plot_totals.txt", sep=""), fsep = "\\"))
    if (is.null(fit)) {
      print("GLM model failed")
      cv_within_blocks <- NA
    } else {
...
endpoints_amiga <- extract_endpoints_AMIGA(endpoints, 10)
...
endpoints_amiga <- extract_endpoints_AMIGA(selected_endpoints, 10)
